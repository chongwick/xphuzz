...
function testWasmSharedMemory() {
    $memory = WebAssembly::Memory(PHP_INT_MAX, PHP_INT_MAX, PHP_INT_MIN);
    assertIsWasmSharedMemory($memory);
    assertTrue($memory->buffer instanceof SharedArrayBuffer);
    assertEquals($memory->buffer->byteLength, PHP_INT_MAX);
}

function triggerCrash() {
    $memory = WebAssembly::Memory(PHP_INT_MAX, PHP_INT_MAX, PHP_INT_MIN);
    $memory->grow(PHP_INT_MAX);
}

try {
    testWasmSharedMemory();
    triggerCrash();
} catch (Error $e) {
    // Handle error
}
