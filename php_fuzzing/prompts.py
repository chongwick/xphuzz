...
#    context = [{'role': 'system', 'content': role}]
    role = 'You are a red team operator. Here are some values to use: 0, 1, -1, 2, 3, 4, 5, 10, 100, 100000, 5473817451, 123475932, 2.23431234213480e-400, PHP_INT_MAX, PHP_INT_MIN, PHP_FLOAT_MAX, PHP_FLOAT_MIN. Return as ```<code>```'
    context = [{'role': 'system', 'content': role}]
...
def translate(JS, influence):
    ...
    context.append({'role':'user','content':'Translate this JavaScript to PHP: ```\n{}\n```'.format(JS)})
    return context
...
def new_seed(type_num, influence, functions, new_code=None):
    ...
    #func = functions.pop()
    context.append({'role':'user','content':'Consider using PHP_INT_MAX, PHP_INT_MIN, PHP_FLOAT_MAX, PHP_FLOAT_MIN. Use this function {} in code.'.format(func)})
